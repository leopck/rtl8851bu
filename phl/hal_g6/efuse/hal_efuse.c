...
enum rtw_hal_status rtw_efuse_renew(void *efuse, u8 type)
{
	...
	if (type == HAL_MP_EFUSE_WIFI) {
		efuse_set_hw_cap(efuse_info);
		rtw_hal_mac_set_xcap(efuse_info->hal_com, 1,
					(u32)efuse_info->hal_com->dev_hw_cap.xcap & 0x7F);
		rtw_hal_mac_set_xcap(efuse_info->hal_com, 0,
					(u32)efuse_info->hal_com->dev_hw_cap.xcap & 0x7F);
		rtw_hal_mac_get_xcap(efuse_info->hal_com, 1, &sc_xo_val);
		rtw_hal_mac_get_xcap(efuse_info->hal_com, 0, &sc_xi_val);
		
		PHL_INFO("Set dev_hw_cap.xcap sc_xi/xo= {0x%x, 0x%x}\n", sc_xi_val, sc_xo_val);
